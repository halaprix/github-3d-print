...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { version, grid, shapeIndex, presetIndex } = decoded;
...
function decodeTokenId(id: bigint): { version: number; grid: number[][]; shapeIndex: number; presetIndex: number } | null {
  ...
  return { version, grid, shapeIndex, presetIndex };
}
